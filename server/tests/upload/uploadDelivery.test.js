...
test("Upload controller returns only safe relative URL", () => {
  ...
  assert.equal(Object.hasOwn(res.body, "destination"), false);
  assert.doesNotMatch(res.body.url, /attacker-controlled/);
  assert.doesNotMatch(res.body.url, /private\/server/);
});
...
test("Static delivery denies hidden dotfiles", async () => {
  ...
  const filename = `.secret-${crypto.randomUUID()}`;
  const filePath = path.join(uploadsRoot, filename);

  await fs.writeFile(filePath, "private-data");
