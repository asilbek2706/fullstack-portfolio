...
const {
  createImageUpload,
} = require("../../middlewares/createImageUpload");
const errorHandler =
...
test("Image upload creates UUID filename for PNG", async () => {
  ...
  assert.equal(
    path.dirname(response.body.file.path),
    uploadPath,
  );

  const files =
    await listUploadedFiles(uploadPath);
